...
const [toggle, setToggle] = useState(false);
  ...
  const slideBlur = useSpring({
    transform: toggle ? "translate(-100%)" : "translate(100%)",
    config: { tension: 300, friction: 20 },
  });
const mouseHover = () =>{
    setPosition({
    ...
    <div className="flex items-center absolute top-1/2 left-1/2 w-full z-20 -translate-x-1/2 -translate-y-1/2 overflow-hidden bg-slate-900 bg-opacity-90 h-screen">
        ...
        (styles , item: boolean) =>
          item && (
              ...
              <animated.div ref={ref}  style={styles}  className="max-w-2xl relative items-center  mx-4  md:mx-auto z-10 bg-slate-100 overflow-hidden justify-around shadow-2xl shadow-slate-900 flex grow shrink-0 py-6 px-4 rounded-2xl">
                <div className= {`${toggle? 'flex': 'hidden' } md:flex flex-col gap-2 py-2 md:py-0 md:px-0 px-10 `}>
                   <Login setLoginModal ={setLoginModal} handleToggle={handleToggle}/>
                </div>
                ...
                >
                    <animated.div style={flipPointAnimation} className=" flex flex-col items-center justify-center h-full">
                    ...
                    >
                        <button className="text-slate-900 font-semibold">{toggle ? "Register": "Login" }</button>
                                            </div>
                    </animated.div>
                    <animated.div
                ref={circlEl}
