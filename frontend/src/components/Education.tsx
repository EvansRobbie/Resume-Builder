      ...
      {(formik) =>(
          <Form className='w-full flex flex-col gap-6'>
              <FieldArray name='education' >
                    {({push, remove}) =>(
                      <div>
                          {formik.values.education.map((_, index) =>(
                            <div key={index} className="my-10 relative group shadow-md overflow-hidden  shadow-slate-900 px-4 rounded-2xl py-5">
                    ...
                    {isEdit && index >= 0 && <Button handleDelete={() =>handleDelete(formik, index)}/>}
                            </div>
                          ))}
